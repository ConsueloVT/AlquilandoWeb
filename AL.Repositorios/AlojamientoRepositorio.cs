...
public class AlojamientoRepositorio : IAlojamientoRepositorio
{
    ...
    public void DeshabilitarPeriodo(int alojamientoId, DateTime desde, DateTime hasta)
    {
        using (var db = new EntidadesContext())
        {
            var nuevoPeriodo = new PeriodoNoDisponible
            {
                AlojamientoId = alojamientoId,
                FechaInicio = desde.Date,
                FechaFin = hasta.Date
            };
            db.PeriodosNoDisponibles.Add(nuevoPeriodo);
            db.SaveChanges();
        }
    }

    public void HabilitarPeriodo(int alojamientoId, DateTime desde, DateTime hasta)
    {
        using (var db = new EntidadesContext())
        {
            var periodos = db.PeriodosNoDisponibles
                .Where(p => p.AlojamientoId == alojamientoId &&
                            p.FechaInicio >= desde.Date &&
                            p.FechaFin <= hasta.Date)
                .ToList();

            db.PeriodosNoDisponibles.RemoveRange(periodos);
            db.SaveChanges();
        }
    }

    public async Task<List<PeriodoNoDisponible>> ObtenerPeriodosNoDisponiblesAsync(int alojamientoId)
    {
        using (var db = new EntidadesContext())
        {
            return await db.PeriodosNoDisponibles
                .Where(p => p.AlojamientoId == alojamientoId)
                .ToListAsync();
        }
    }
    ...
    public async Task AgregarPeriodoNoDisponible(int alojamientoId, PeriodoNoDisponible periodo)
    {
        using (var db = new EntidadesContext())
        {
            var alojamiento = await db.Alojamientos
                .Include(a => a.FechasNoDisponibles)
                .FirstOrDefaultAsync(a => a.Id == alojamientoId);

            if (alojamiento == null)
                throw new Exception("Alojamiento no encontrado.");

            alojamiento.FechasNoDisponibles.Add(periodo);

            await db.SaveChangesAsync();
        }
    }

    public async Task EliminarPeriodoNoDisponible(int alojamientoId, PeriodoNoDisponible periodo)
    {
        using (var db = new EntidadesContext())
        {
            var alojamiento = await db.Alojamientos
            ...
            alojamiento.FechasNoDisponibles.RemoveAll(
                p => p.FechaInicio == periodo.FechaInicio && p.FechaFin == periodo.FechaFin
            );
